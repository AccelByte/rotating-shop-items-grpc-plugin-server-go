...
RUN go build -v -modcacherw -o /output/$TARGETOS/$TARGETARCH/rotating-shop-items-grpc-plugin-server-go .
...
# Copy build
COPY --from=builder /output/$TARGETOS/$TARGETARCH/rotating-shop-items-grpc-plugin-server-go rotating-shop-items-grpc-plugin-server-go
