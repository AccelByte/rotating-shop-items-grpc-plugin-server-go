...
RUN env GOOS=$TARGETOS GOARCH=$TARGETARCH go build -modcacherw -o rotating-shop-items-grpc-plugin-server-go_$TARGETOS-$TARGETARCH
...
COPY --from=builder /build/rotating-shop-items-grpc-plugin-server-go_$TARGETOS-$TARGETARCH rotating-shop-items-grpc-plugin-server-go
...
# Entrypoint.
CMD [ "/app/rotating-shop-items-grpc-plugin-server-go" ]